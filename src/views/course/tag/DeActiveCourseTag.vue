...
<script>
    ...
    export default {
        ...
        created() {
            this.$store.state.pageTitle = 'لیست تگ ها غیر فعال دوره ها';
            this.getListOfTheDeActiveTags();
        },
        methods: {
            searchInTable() {
                ...
                this.last_page=0;
                this.$store.state.loading = true;
                TagService.searchInDeActiveTags(this.search_value)
                    .then((res) => {
                        let list = [];
                        if (res.status === 204) {
                            this.rows = [];
            ...
            getListOfTheDeActiveTags() {
                this.$store.state.loading = true;
                TagService.getListOfTheDeActiveTags(this.current)
                    .then(res => {
                        let list = [];
                        if (res.status === 204) {
                            this.rows = list;
