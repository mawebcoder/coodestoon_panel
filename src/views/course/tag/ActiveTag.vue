    ...
    export default {
        ...
        watch: {
            current(value) {
                ...
                TagService.paginateListOfTheActiveCourseTags(value)
                    .then(res => {
                        let list = [];
        ...
        created() {
            this.$store.state.pageTitle = 'لیست تگ ها فعال دوره ها';
            this.getListOfTheActiveCourseTags();
        },
        methods: {
            searchInTable() {
                ...
                this.last_page=0;
                this.$store.state.loading = true;
                TagService.searchInActiveTags(this.search_value)
                    .then((res) => {
                        let list = [];
                        if (res.status === 204) {
                            this.rows = [];
            ...
            getListOfTheActiveCourseTags() {
                this.$store.state.loading = true;
                TagService.getListOfTheActiveCourseTags(this.current)
                    .then(res => {
                        let list = [];
                        if (res.status === 204) {
                            this.rows = list;
