...
export default {
  ...
  created() {
    ...
    this.$store.state.pageTitle = 'لیست دوره های غیر فعال';
    this.getDeActiveCourses()
  },
  data() {
    return {
  ...
  watch: {
    current(value) {
      this.$store.state.loading = true;
      CourseService.paginateDeActiveCourses(value)
          .then(res => {
            this.rows=[];
  ...
  methods: {
    ...
    getDeActiveCourses() {
      this.rows = [];
      CourseService.getDeActiveCourses()
          .then(res => {
            if (res.status === 204) {
              this.rows = [];
              this.last_page = 0;
    ...
    searchInTable() {
      ...
      this.$store.state.loading = true;
      CourseService.searchInDeActiveCourses(this.search_value)
          .then((res) => {
            if (res.status === 204) {
              this.rows = [];
              this.$store.state.loading = false;
