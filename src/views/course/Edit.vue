<template>
  <div>
    ...
    <div class="form-group" dir="ltr" style="height: 19px">
      <p-check color="success" v-model="is_completed_course" class="p-switch p-fill"></p-check>
    </div>
    <span
        :class="[is_completed_course ? 'active_color' : 'de_active_color']">{{
        isCompletedCourse
      }}</span>
...
export default {
  ...
  computed: {
    ...
    isCompletedCourse() {
      return this.is_completed_course ? 'بله' : 'خیر'
    }
  },
  methods: {
    getCourseInformation() {
      CourseService.getCourseInformation(this.$route.params.id)
          .then(res => {
            this.setInformationValues(res);
          }).catch(error => {
        HelperClass.showErrors(error, this.$noty)
      })
    },
    setInformationValues(res) {


      let course_information = res.data.data.course;

      let synced_tags = res.data.data.synced_tags;

      this.selected_tags = [];

      if (synced_tags.length) {
        synced_tags.forEach(item => {
          this.selected_tags.push(item.id)
        })
      }
      this.category = course_information.courseCategory_id;
      this.meta = course_information.meta
      this.en_title = course_information.en_title;
      this.fa_title = course_information.fa_title;
