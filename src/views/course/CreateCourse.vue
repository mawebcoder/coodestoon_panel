<template>
    <div>
        ...
        <span :class="[sell_to_special_memberships ? 'active_color' : 'de_active_color']">{{sellToSpecialMembershipCondition}}</span>
        ...
        methods: {
            ...
            getTags() {
                TagService.getListOfTheTags('select_box')
                    .then(res => {
                        if (res.status === 204) {
                            this.tags.unshift({value: 0, text: 'هیچ تگی ثب نشده است'})
                            return;
                        }
