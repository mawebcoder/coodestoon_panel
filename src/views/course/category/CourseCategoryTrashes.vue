<template>
    <div>
        ...
        <span  title="بازیابی موارد انتخابی" style="display: inline-block;background-color:rgba(54,166,184,0.95);"
              @click="restoreCourseCategory" class="delete_button">
...
<script>
    ...
    export default {
        ...
        created() {
            ...
            this.$store.state.pageTitle = 'لیست دسته بندی دوره ها';
            this.getTrashedCourseCategories();
        },
        data() {
            return {
        ...
        watch: {
            current(value) {
                this.$store.state.loading = true;
                CourseCategoryService.paginateDeletedCourseCategories(value)
                    .then(res => {
                        let list = [];
        ...
        methods: {
            getTrashedCourseCategories() {
                CourseCategoryService.getTrashedCourseCategories()
                    .then(res => {
                        let list = [];
                        if (res.status === 204) {
                            this.rows = list;
            ...
            searchInTable() {
                ...
                this.$store.state.loading = true;
                CourseCategoryService.searchInDeletedCourseCategories(this.search_value)
                    .then((res) => {
                        let list = [];
                        if (res.status === 204) {
                            this.rows = [];
            ...
            deleteItems() {
                ...
                HelperClass.showSwalAsking().then(result => {
                    ...
                    if (result.value) {
                        ...
                        CourseCategoryService.forceDeleteCourseCategories(data)
                            .then(() => {
                                this.selected = [];
                                if (this.current > 1) {
            ...
            restoreCourseCategory(){
                ...
                HelperClass.showSwalAsking().then(result => {
                    ...
                    if (result.value) {
                        ...
                        CourseCategoryService.restoreCourseCategories(data)
                            .then(() => {
                                this.selected = [];
                                if (this.current > 1) {
                                    this.current = this.current - 1;
