...
<script>
    ...
    export default {
        ...
        created() {
            ...
            this.$store.state.pageTitle = 'لیست دسته بندی های فعال مقالات';
            this.getListOfTheActiveArticleCategories();
        },
        data() {
            return {
        ...
        watch: {
            current(value) {
                this.$store.state.loading = true;
                ArticleService.paginateActiveArticleCategories(value)
                    .then(res => {
                        let list = [];
        ...
        methods: {
            getListOfTheActiveArticleCategories() {
                ArticleService.getListOfTheActiveArticleCategories(1)
                    .then(res => {
                        let list = [];
                        if (res.status === 204) {
                            this.rows = list;
            ...
            searchInTable() {
                ...
                this.$store.state.loading = true;
                ArticleService.searchInActiveArticleCategories(this.search_value)
                    .then((res) => {
                        let list = [];
                        if (res.status === 204) {
                            this.rows = [];
            ...
            onCellClick(params) {
                ...
                switch (tagName) {
                    case ("BOX-ICON"):
                        ...
                        if (column_name === 'edit') {
                            this.$router.push({name:'article-category-edit',params:{articleCategory:row_object.id},query:{from_actives:true}})
