  ...
  <div>
    ...
    <template v-if="show_pagination">
      <div dir="ltr" style="margin-top: 20px;margin-bottom: 100px">
        ...
        <!--the total number of the 6 and 7 has bug-->
...
export default {
  ...
  created() {
    ...
    this.$store.state.pageTitle = 'لیست دسته بندی مقالات';
  ...
  watch: {
    current(value) {
      this.$store.state.loading = true;
      CategoryService.paginateArticleCategories(value)
          .then(res => {
            let list = [];
  ...
  methods: {
    getListOfTheArticleCategories() {
      CategoryService.getListOfTheArticleCategories(1)
          .then(res => {
            let list = [];
            if (res.status === 204) {
              this.rows = list;
    ...
    searchInTable() {
      ...
      this.$store.state.loading = true;
      CategoryService.searchInArticleCategories(this.search_value)
          .then((res) => {
            let list = [];
            if (res.status === 204) {
              this.rows = [];
    ...
    onCellClick(params) {
      ...
      switch (tagName) {
        case ("BOX-ICON"):
          ...
          if (column_name === 'edit') {
            this.$router.push({name: 'article-category-edit', params: {articleCategory: row_object.id}})
