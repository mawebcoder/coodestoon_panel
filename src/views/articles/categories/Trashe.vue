<template>
    <div>
        ...
        <span title="بازیابی موارد انتخابی" style="display: inline-block;background-color:rgba(54,166,184,0.95);" @click="restoreItems" class="delete_button">
...
<script>
    ...
    export default {
        ...
        watch: {
            current(value) {
                this.$store.state.loading = true;
                ArticleService.paginateTrashesArticleCategories(value)
                    .then(res => {
                        let list = [];
        ...
        methods: {
            getListOfTheTrashesArticleCategories() {
                ArticleService.getListOfTheTrashesArticleCategories(1)
                    .then(res => {
                        let list = [];
                        if (res.status === 204) {
                            this.rows = list;
            ...
            searchInTable() {
                ...
                this.$store.state.loading = true;
                ArticleService.searchInTrashesArticleCategories(this.search_value)
                    .then((res) => {
                        let list = [];
                        if (res.status === 204) {
                            this.rows = [];
            ...
            deleteItems() {
                ...
                HelperClass.showSwalAsking().then(result => {
                    ...
                    if (result.value) {
                        ...
                        ArticleService.forceDeleteArticleCategory(data)
                            .then(() => {
                                this.selected = [];
                                if (this.current > 1) {
            ...
            restoreItems(){
                ...
                HelperClass.showSwalAsking().then(result => {
                    ...
                    if (result.value) {
                        ...
                        ArticleService.restoreArticleCategories(data)
                            .then(() => {
                                this.selected = [];
                                if (this.current > 1) {
                                    this.current = this.current - 1;
