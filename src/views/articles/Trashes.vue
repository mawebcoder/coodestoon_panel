...
<script>
    ...
    export default {
        ...
        created() {
            ...
            this.$store.state.pageTitle = 'سطل زباله مقالات';
            this.getArticlesTrashes();
        },
        data() {
            return {
        ...
        watch: {
            current(value) {
                this.$store.state.loading = true;
                ArticleService.paginateTrashedArticles(value)
                    .then(res => {
                        let list = [];
        ...
        methods: {
            getArticlesTrashes() {
                ArticleService.getArticlesTrashes()
                    .then(res => {
                        let list = [];
                        if (res.status === 204) {
                            this.rows = list;
            ...
            searchInTable() {
                ...
                this.$store.state.loading = true;
                ArticleService.searchInTrashedArticles(this.search_value)
                    .then((res) => {
                        let list = [];
                        if (res.status === 204) {
                            this.rows = [];
            ...
            deleteItems() {
                ...
                HelperClass.showSwalAsking().then(result => {
                    ...
                    if (result.value) {
                        ...
                        ArticleService.forceDeleteArticles(data)
                            .then(() => {
                                this.selected = [];
                                if (this.current > 1) {
            ...
            restoreItems(){
                ...
                HelperClass.showSwalAsking().then(result => {
                    ...
                    if (result.value) {
                        ...
                        ArticleService.restoreArticles(data)
                            .then(() => {
                                this.selected = [];
                                if (this.current > 1) {
                                    this.current = this.current - 1;
