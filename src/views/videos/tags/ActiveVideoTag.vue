...
export default {
  ...
  watch: {
    current(value) {
      ...
      VideosTagService.paginateActiveVideoTags(value)
          .then(res => {
            let list = [];
  ...
  methods: {
    searchInTable() {
      ...
      this.$store.state.loading = true;
      VideosTagService.searchInActiveVideoTags(this.search_value)
          .then((res) => {
            let list = [];
            if (res.status === 204) {
              this.rows = [];
    ...
    getList() {
      this.$store.state.loading = true;
      VideosTagService.getActivesVideoTags(this.current)
          .then(res => {
            let list = [];
            if (res.status === 204) {
              this.rows = list;
