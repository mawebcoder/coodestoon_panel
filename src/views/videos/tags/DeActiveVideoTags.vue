...
export default {
  ...
  watch: {
    current(value) {
      ...
      VideosTagService.paginateDeActiveVideoTags(value)
          .then(res => {
            let list = [];
  ...
  methods: {
    searchInTable() {
      ...
      this.$store.state.loading = true;
      VideosTagService.searchInDeActiveVideoTags(this.search_value)
          .then((res) => {
            let list = [];
            if (res.status === 204) {
              this.rows = [];
    ...
    update() {
      ...
      VideosTagService.updateVideoTagService(this.id, data)
          .then(() => {
            this.getList();
    ...
    getList() {
      this.$store.state.loading = true;
      VideosTagService.getDeActivesVideoTags(this.current)
          .then(res => {
            let list = [];
            if (res.status === 204) {
              this.rows = list;
    ...
    switchCondition(row) {
      ...
      VideosTagService.switchActivation(row.id, data)
          .then(() => {
            this.getList();
            HelperClass.showSuccess(this.$noty)
          }).catch(error => {
    ...
    deleteItems() {
      ...
      HelperClass.showSwalAsking().then(result => {
        ...
        if (result.value) {
          ...
          VideosTagService.deleteVideoTags(data)
              .then(() => {
                this.selected = [];
                if (this.current > 1) {
