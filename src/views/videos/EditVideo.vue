...
export default {
  ...
  methods: {
    setValues(data) {
      ...
      // set the values to show user
      this.fa_title = video_info.fa_title;
      this.en_title = video_info.en_title;
      this.content = video_info.description;
      this.is_single_video = video_info.is_single_video
      this.is_free = video_info.is_free;
      this.minute = time.split(':')[0]
      if (typeof time.split(':')[1] !== 'undefined') {
        this.second = time.split(':')[1]
      }

      this.meta_description = video_info.meta;
      this.short_description = video_info.short_description
      this.status = video_info.status;


      if (video_info.course_id != null) {
        this.course_id = video_info.course_id;
      }
      if (video_info.course_section_id != null) {
        this.course_section_id = video_info.courseSection_id;
      }
      if (tags.length) {
        let tag_ids = [];
        tags.forEach(item => {
    ...
    getVideoInformation() {
      VideosService.getVideoInformations(this.$route.params.id)
          .then(res => {
    ...
    getValues() {
      ...
      if (this.is_single_video) {
        ...
        Object.assign(data, {
          courseSection_id: this.course_section_id ? this.course_section_id : null,
    ...
    submit() {
      ...
      VideosService.updateVideo(this.$route.params.id,data)
          .then((res) => {
