...
export default {
  ...
  watch: {
    current(value) {
      ...
      VideosService.paginateInDeActiveVideos(value)
          .then(res => {
            this.rows = [];
  ...
  created() {
    this.$store.state.loading = true;
    this.$store.state.pageTitle = 'لیست قسمت های فعال';
    this.getList()
  },
  methods: {
    ...
    getList() {
      this.rows = [];
      VideosService.getDeActiveVideos()
          .then(res => {
            if (res.status === 204) {
              this.rows = [];
              this.last_page = 0;
    ...
    searchInTable() {
      ...
      this.$store.state.loading = true;
      VideosService.searchInDeActiveVideos(this.search_value)
          .then((res) => {
            if (res.status === 204) {
              this.rows = [];
              this.$store.state.loading = false;
