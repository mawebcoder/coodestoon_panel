...
export default {
  ...
  watch: {
    current(value) {
      ...
      VideosService.paginateInVideoTrashes(value)
          .then(res => {
            this.rows = [];
  ...
  methods: {
    ...
    searchInTable() {
      ...
      this.$store.state.loading = true;
      VideosService.searchInTrashedVideos(this.search_value)
          .then((res) => {
            if (res.status === 204) {
              this.rows = [];
              this.$store.state.loading = false;
    ...
    deleteItems() {
      ...
      HelperClass.showSwalAsking().then(result => {
        ...
        if (result.value) {
          ...
          VideosService.videosForceDelete(data)
              .then(() => {
                this.selected = [];
                if (this.current > 1) {
    ...
    restoreItems(){
      ...
      HelperClass.showSwalAsking().then(result => {
        ...
        if (result.value) {
          ...
          VideosService.restoreVideos(data)
              .then(() => {
                this.selected = [];
                if (this.current > 1) {
                  this.current = this.current - 1;
