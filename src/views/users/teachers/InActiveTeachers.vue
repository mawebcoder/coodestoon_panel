...
export default {
  ...
  watch: {
    current(value) {
      this.$store.state.loading = true;
      UserService.paginateInInActiveTeachers(value)
          .then(res => {
            let list = [];
              ...
              list.push(
                  {
                    ...
                    email:item.email,
                    cell:item.cell,
                    course:"<span class='active_it'><box-icon name='show-alt'></box-icon></span>",
  ...
  methods: {
    getList() {
      UserService.getInActiveTeachers()
          .then(res => {
            let list = [];
            if (res.status === 204) {
              this.rows = list;
    ...
    searchInTable() {
      ...
      this.$store.state.loading = true;
      UserService.searchInInActiveTeachers(this.search_value)
          .then((res) => {
            let list = [];
            if (res.status === 204) {
              this.rows = [];
    ...
    onCellClick(params) {
      ...
      switch (tagName) {
        case ("BOX-ICON"):
          ...
          if (column_name === 'edit') {
            this.$router.push({name: 'users-teachers-edit',params: {teacher_id: row_object.id},query:{from_list:true}})
